function [DD,BB,F]=train0(XTrain_t,YTrain_t,param,anchor)
...
%     dX = size(XTrain_t,1);
    ...
    %% iterative optimization
        for iter = 1:param.iter
            ...
            % update W
            W1=theta1*(XTrain_t*H_new')/(theta1*(H_new*H_new')+lamda*eye(nbits)) ;
            W2=theta2*(YTrain_t*H_new')/(theta2*(H_new*H_new')+lamda*eye(nbits)) ;
            
            % update R
            R=(kesi*(L_new*YTrain_t')+yita*(YTrain_t*YTrain_t'))/((kesi+yita)*(YTrain_t*YTrain_t'));
 
            % update L_new
            L_new=((kesi+sigma)*eye(dY))^(-1)*(kesi*R*YTrain_t+sigma*P*H_new);
            
            %update F
           F=(B*XTrain_t')/(XTrain_t*XTrain_t'+lamda*eye(dX));
           
           %update P
           P=(L_new*H_new')/(H_new*H_new'+lamda*eye(nbits));
           
           %update B
           B = sign(alpha*H_new+beta*nbits*(H_new*L_new')*L_new+phi*F*XTrain_t);
